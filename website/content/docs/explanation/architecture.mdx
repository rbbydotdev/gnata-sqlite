...
│         (Vite + React, two modes)               │
├─────────────────────┬───────────────────────────┤
│  @gnata-sqlite/react│     sql.js + gnata ext    │
│  (hooks, components)│     (WASM SQLite with     │
├─────────────────────┤      loadable extension)  │
│  @gnata-sqlite/     │                           │
│    codemirror       │                           │
│  (sync HL + async   │                           │
│   WASM LSP)         │                           │
├─────────────────────┼───────────────────────────┤
│  gnata-lsp.wasm     │     sqlite/ extension     │
│  (TinyGo, 182KB)    │     (CGo, c-shared)       │
├─────────────────────┴───────────────────────────┤
│            gnata core engine (Go)               │
│    GJSON fast path  ←→  full AST evaluator      │
│         single dep: tidwall/gjson               │
└─────────────────────────────────────────────────┘
         ↑ schema JSON flows down from backend
```
...
**Why TinyGo?** Standard Go WASM: ~5.3 MB. TinyGo: 182 KB (27x smaller). For a module that loads on every page visit, that is the difference between instant and a loading spinner.
...
4. LSP in browsers → TinyGo WASM (182 KB)
