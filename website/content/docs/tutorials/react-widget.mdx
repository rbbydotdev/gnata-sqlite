...
export default function App() {
  ...
  if (!wasm.isReady) return <div>Loading WASM...</div>
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
      <JsonataEditor
        ...
        schema={schema} wasm={wasm} theme="dark"
...
function useMyEditor() {
  ...
  const { result, error, timing } = useJsonataEval(expr, input, wasm)
...
No `wasm` prop = no WASM loading. Highlighting works instantly with the tokenizer only.

## Next Steps

- [React Widget API Reference](/reference/react-widget) — full props, hooks, and types
- [Editor & LSP API](/reference/editor-api) — low-level CodeMirror and WASM exports
- [TinyGo WASM](/explanation/tinygo-wasm) — why the LSP is 85KB
